...
public class ParticleSystem {
	 ...
	 */
	public void emit (int x, int y, int particlesPerSecond, int emitingTime) {
		// Setup emiter
		configureEmiter(x,y);
	 ...
	 * @param particlesPerSecond Number of particles per second that will be emited (evenly distributed)
	 */
	public void emit (int x, int y, int particlesPerSecond) {
		// Setup emiter
		configureEmiter(x,y);
		mActivatedParticles = 0;
		mParticlesPerMilisecond = particlesPerSecond/1000f;
	...
	public void oneShot(int x, int y, int numParticles) {
		oneShot(x, y, numParticles, new LinearInterpolator());
	}
	...
	private void configureEmiter(int x, int y) {
